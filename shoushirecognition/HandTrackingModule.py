...
class HandDetector:
    ...
    def findHands(self, img, draw=True):
        ...
        self.results = self.hands.process(imgRGB) #把图像传入检测模型，提取信息

        if self.results.multi_hand_landmarks:
            for handLms in self.results.multi_hand_landmarks:# 获取每个关键点的索引和坐标
                if draw:
                    self.mpDraw.draw_landmarks(img, handLms,
                                             self.mpHands.HAND_CONNECTIONS)#绘制每只手的关键点,mpHands.HAND_CONNECTIONS绘制手部关键点之间的连线
    ...
    def findPosition(self, img, handNo=0, draw=True):
        ...
        if self.results.multi_hand_landmarks:
            for handLms in self.results.multi_hand_landmarks:
                for id, lm in enumerate(handLms.landmark):
                    ...
                    cx, cy = int(lm.x * w), int(lm.y * h)  # 将归一化坐标转换为像素坐标
                    if id == 8:  # 8 是食指指尖的索引
                        xList.append(cx)
                        yList.append(cy)
                        #print(f"指定关键点 {id} 的像素坐标为：({cx}, {cy})")
        ...
        return x,y,self.lmList, bboxInfo
